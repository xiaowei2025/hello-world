...
public class ActivitiHolidayController {
    ...
    public ResponseEntity getDeployList(){
        HashMap<String, Object> resultMap = new HashMap<>(8);
        ProcessEngine processEngine = ProcessEngines.getDefaultProcessEngine();

        // 使用RepositoryService来访问流程定义信息
//        org.activiti.engine.repository.RepositoryService repositoryService = processEngine.getRepositoryService();
        ...
        // 遍历结果列表并打印每个流程定义的ID和名称
        for (ProcessDefinition processDefinition : processDefinitions) {


            resultMap.put("流程部署id:", processDefinition.getId());
            resultMap.put("流程部署名称-name:", processDefinition.getName());
            resultMap.put("流程部署key", processDefinition.getKey());
            resultMap.put("Process Definition Version: " ,processDefinition.getVersion());
            resultMap.put("Deployment ID: " , processDefinition.getDeploymentId());
//            resultMap.put("Resource Name: " , getResourceNameByDeploymentIdAndProcessDefinitionKey(repositoryService, processDefinition.getDeploymentId(), processDefinition.getKey()));
...
//            System.out.println("Deployment ID: " + processDefinition.getDeploymentId());
//            System.out.println("Resource Name: " + getResourceNameByDeploymentIdAndProcessDefinitionKey(repositoryService, processDefinition.getDeploymentId(), processDefinition.getKey()));
//            System.out.println("---------------------------");
        }
        return ResponseEntity.ok(resultMap);
    }

//    private String getResourceNameByDeploymentIdAndProcessDefinitionKey(RepositoryService repositoryService, String deploymentId, String processDefinitionKey) {
//        List<Deployment> deployments = repositoryService.createDeploymentQuery().deploymentId(deploymentId).list();
//        if (!deployments.isEmpty()) {
//            String resourceName = deployments.get(0).getResources().stream()
//                    .filter(resource -> resource.getName().endsWith(".bpmn.xml")) // 假设你的BPMN文件是以.bpmn20.xml结尾的
//                    .filter(resource -> resource.getResourceName().contains(processDefinitionKey)) // 过滤出包含流程定义键的资源名
//                    .findFirst()
//                    .map(org.activiti.engine.repository.Resource::getResourceName)
//                    .orElse(null);
//            return resourceName;
//        }
//        return null;
//    }
    ...
    public String getTaskByUserName(@RequestParam(required = false) String userName, Model model) {
        ...
        taskList.forEach(task -> {
            ...
            map.put("title",holiday.getTitle());
